...
def modelos_classificacao(df: pd.DataFrame) -> dict:
    ...
    if HAS_CAT:
        modelos["CatBoost"] = CatBoostClassifier(iterations=100, random_state=42, verbose=0, allow_writing_files=False)
    ...
    if HAS_CAT:
        try:
            cat_m = CatBoostRegressor(
                ...
                depth=6, random_seed=42, verbose=0, allow_writing_files=False
            )
...
# #############################################################################
# ##                                                                         ##
# ##   EXPANSÃO v1.3 — SIPREV / DENGUE CAMPO GRANDE-MS                       ##
# ##   Catálogos de 300 bibliotecas (100 ML · 100 DL · 100 NN) · Módulos de  ##
# ##   RNN · ANN · NLP · Orquestrador v4                                     ##
# ##                                                                         ##
# #############################################################################
# #############################################################################
...
# =============================================================================
# SEÇÃO 181 ─ CATÁLOGOS EXPANDIDOS PARA 100 BIBLIOTECAS (ML · DL · NN)
# =============================================================================
# Acrescenta 20 bibliotecas a cada catálogo (80 → 100), totalizando 300.
# Reutiliza as listas CATALOGO_*_80 da v1.2.
# -----------------------------------------------------------------------------

CATALOGO_ML_EXTRA_20 = [
    ("pyspark", "PySpark MLlib", "ML distribuído em Apache Spark"),
    ("Orange", "Orange3", "ML visual e mineração de dados"),
    ("mlpack", "mlpack", "ML em C++ com bindings Python"),
    ("shogun", "Shogun", "Biblioteca de ML de larga escala"),
    ("ngboost", "NGBoost", "Boosting probabilístico (natural gradient)"),
    ("pygam", "pyGAM", "Modelos aditivos generalizados (GAM)"),
    ("sksurv", "scikit-survival", "Análise de sobrevivência com sklearn"),
    ("pyearth", "py-earth (MARS)", "Splines de regressão adaptativa"),
    ("rgf", "Regularized Greedy Forest", "Florestas gulosas regularizadas"),
    ("glum", "glum", "GLMs rápidos e regularizados"),
    ("pyglmnet", "pyglmnet", "GLMs elásticos (elastic-net)"),
    ("autogluon", "AutoGluon", "AutoML para tabular/séries/visão"),
    ("kats", "Kats", "Análise de séries temporais (Meta)"),
    ("merlion", "Merlion", "Previsão e detecção de anomalias"),
    ("tsfel", "TSFEL", "Extração de features de séries"),
    ("skrub", "skrub", "Preparação de dados para ML"),
    ("snapml", "Snap ML", "ML acelerado (IBM)"),
    ("mord", "mord", "Regressão ordinal"),
    ("sklego", "scikit-lego", "Extensões úteis do scikit-learn"),
    ("pmdarima", "pmdarima (auto)", "Auto-ARIMA (reforço de catálogo)"),
]

CATALOGO_DL_EXTRA_20 = [
    ("sonnet", "DeepMind Sonnet", "Módulos de NN sobre TensorFlow"),
    ("tensorlayer", "TensorLayer", "DL modular sobre TensorFlow"),
    ("tflearn", "TFLearn", "API de alto nível para TensorFlow"),
    ("dynet", "DyNet", "Rede neural dinâmica"),
    ("singa", "Apache SINGA", "DL distribuído"),
    ("oneflow", "OneFlow", "Framework de DL de alto desempenho"),
    ("mindspore", "MindSpore", "Framework de DL (Huawei)"),
    ("megengine", "MegEngine", "Framework de DL (Megvii)"),
    ("ivy", "Ivy", "DL unificado entre frameworks"),
    ("nnabla", "NNabla", "Neural Network Libraries (Sony)"),
    ("composer", "MosaicML Composer", "Treino eficiente de DL"),
    ("colossalai", "Colossal-AI", "Treino de modelos gigantes"),
    ("fairscale", "FairScale", "Escalonamento de treino (Meta)"),
    ("xformers", "xFormers", "Blocos de Transformer eficientes"),
    ("flash_attn", "FlashAttention", "Atenção rápida em memória"),
    ("torchdata", "TorchData", "Pipelines de dados para PyTorch"),
    ("stable_baselines3", "Stable-Baselines3", "Aprendizado por reforço"),
    ("tianshou", "Tianshou", "Aprendizado por reforço (PyTorch)"),
    ("jraph", "Jraph", "Graph nets em JAX"),
    ("lightning_bolts", "Lightning Bolts", "Modelos prontos (Lightning)"),
]

CATALOGO_NN_EXTRA_20 = [
    ("torch_geometric_temporal", "PyG Temporal", "GNN espaço-temporais"),
    ("deepsnap", "DeepSNAP", "Suporte a GNN sobre PyG"),
    ("ogb", "Open Graph Benchmark", "Benchmarks de GNN"),
    ("karateclub", "Karate Club", "Embeddings de grafos/nós"),
    ("node2vec", "node2vec", "Embeddings de nós"),
    ("pykeen", "PyKEEN", "Embeddings de grafos de conhecimento"),
    ("ampligraph", "AmpliGraph", "Embeddings de KG (TF)"),
    ("torchdrug", "TorchDrug", "Redes para descoberta de fármacos"),
    ("e3nn", "e3nn", "Redes equivariantes (E(3))"),
    ("pytorch_metric_learning", "PyTorch Metric Learning", "Aprendizado métrico"),
    ("pytorch_optimizer", "pytorch-optimizer", "Otimizadores avançados"),
    ("adabelief_pytorch", "AdaBelief", "Otimizador AdaBelief"),
    ("lion_pytorch", "Lion", "Otimizador Lion"),
    ("torchsde", "torchsde", "Equações diferenciais estocásticas"),
    ("torchdiffeq", "torchdiffeq", "Neural ODEs"),
    ("torchcde", "torchcde", "Neural CDEs (séries irregulares)"),
    ("deepxde", "DeepXDE", "Redes informadas por física (PINNs)"),
    ("neurodiffeq", "neurodiffeq", "EDOs/EDPs com redes neurais"),
    ("escnn", "escnn", "Redes equivariantes a grupos"),
    ("graphein", "Graphein", "Grafos de proteínas/moléculas"),
]

# Catálogos completos (100 por categoria) — reutilizam as listas da v1.2.
CATALOGO_ML_100 = CATALOGO_ML_80 + CATALOGO_ML_EXTRA_20
CATALOGO_DL_100 = CATALOGO_DL_80 + CATALOGO_DL_EXTRA_20
CATALOGO_NN_100 = CATALOGO_NN_80 + CATALOGO_NN_EXTRA_20


def catalogo_machine_learning_100() -> pd.DataFrame:
    """Catálogo de 100 bibliotecas de Machine Learning."""
    return _catalogar_bibliotecas(
        CATALOGO_ML_100,
        "🤖 MACHINE LEARNING — 100 BIBLIOTECAS (GRANDES MODELOS)",
        "catalogo_ml_100", cor="#1F618D")


def catalogo_deep_learning_100() -> pd.DataFrame:
    """Catálogo de 100 bibliotecas de Deep Learning."""
    return _catalogar_bibliotecas(
        CATALOGO_DL_100,
        "🧠 DEEP LEARNING — 100 BIBLIOTECAS (GRANDES MODELOS)",
        "catalogo_dl_100", cor="#641E16")


def catalogo_neural_networks_100() -> pd.DataFrame:
    """Catálogo de 100 bibliotecas de Neural Networks."""
    return _catalogar_bibliotecas(
        CATALOGO_NN_100,
        "🧬 NEURAL NETWORKS — 100 BIBLIOTECAS (GRANDES MODELOS)",
        "catalogo_nn_100", cor="#4A235A")


def compilar_300_bibliotecas() -> dict:
    """
    Compila os três catálogos de 100 bibliotecas (100 ML + 100 DL + 100 NN =
    300), exibe inline, exporta e gera um resumo consolidado.
    """
    banner_inline("COMPILADO DE 300 BIBLIOTECAS (100 ML · 100 DL · 100 NN)",
                  cor="#0B5345")
    df_ml = catalogo_machine_learning_100()
    df_dl = catalogo_deep_learning_100()
    df_nn = catalogo_neural_networks_100()
    ...
    tab = make_table(["Categoria", "Catalogadas", "Instaladas", "Ausentes"],
                     resumo, col_align=["l", "r", "r", "r"])
    exibir_tabela_inline(tab, titulo="Resumo dos 300 Catálogos de Bibliotecas")
    salvar_tabela_txt(tab, "catalogo_300_resumo",
                      "Compilado de 300 Bibliotecas (100 ML · 100 DL · 100 NN)")
    salvar_tabela_log(tab, "catalogo_300_resumo", "300 Bibliotecas")
    ...
    try:
        df_all = pd.concat([
            ...
            df_dl.assign(Categoria="Deep Learning"),
            df_nn.assign(Categoria="Neural Networks")], ignore_index=True)
        df_all.to_csv(OUTPUT_DIR / "dados" / "catalogo_300_bibliotecas.csv",
                      index=False, encoding="utf-8-sig")
        if HAS_OPENPYXL:
            with pd.ExcelWriter(
                    OUTPUT_DIR / "dados" / "catalogo_300_bibliotecas.xlsx") as xls:
                df_ml.to_excel(xls, sheet_name="Machine Learning", index=False)
                df_dl.to_excel(xls, sheet_name="Deep Learning", index=False)
                df_nn.to_excel(xls, sheet_name="Neural Networks", index=False)
        log.info("  [EXPORT] catalogo_300_bibliotecas.csv/.xlsx")
    except Exception as e:
        log.warning(f"  Exportação 300: {e}")
    return {"ml": df_ml, "dl": df_dl, "nn": df_nn}


PIPELINE_MODULES_V2.update({
    "catalogo_ml_100":  "catalogo_machine_learning_100",
    "catalogo_dl_100":  "catalogo_deep_learning_100",
    "catalogo_nn_100":  "catalogo_neural_networks_100",
    "catalogo_300":     "compilar_300_bibliotecas",
})


# =============================================================================
# SEÇÃO 182 ─ RECURRENT NEURAL NETWORKS (RNNs) — GRANDES MODELOS
# =============================================================================
# Suíte dedicada de Redes Neurais Recorrentes (PyTorch) para previsão da série
# mensal de casos: RNN simples (Elman), RNN empilhada, RNN bidirecional, RNN com
# atenção e híbrido LSTM→GRU. Contrato (B, janela) -> (B, 1).
# -----------------------------------------------------------------------------

CAT_RNN = "Recurrent Neural Network (RNN)"
CAT_ANN = "Artificial Neural Network (ANN)"
CAT_NLP = "Natural Language Processing (NLP)"


def _treinar_modelo_serie(serie: pd.Series, escopo: str, nome: str, classe,
                          cfg: dict, categoria: str, prefixo: str,
                          janela: int = 12, horizonte: int = 12) -> dict:
    """Treina/avalia/plota/registra um modelo de série temporal (PyTorch)."""
    if not HAS_TORCH:
        return {}
    vals = serie.values.astype(float)
    if len(vals) < janela + 6:
    ...
    try:
        modelo = classe(janela)
        modelo, _ = _treinar_torch_seq(modelo, X_tr, y_tr, X_te, y_te,
                                       epochs=cfg.get("epochs", 140),
                                       lr=cfg.get("lr", 0.01), nome=nome)
    except Exception as e:
        log.error(f"  {nome}: falha no treino — {e}")
        return {}
    modelo.eval()
    ...
    nome_arq = prefixo + re.sub(r"[^a-z0-9]+", "_", nome.lower()).strip("_")
    fig, ax = plt.subplots(figsize=(13, 5))
    ax.plot(range(len(vals)), vals, color=COR_PRINCIPAL, linewidth=2,
            label="Histórico")
    ax.plot(range(len(vals), len(vals) + horizonte), fut, color=COR_VERDE,
            linewidth=2, linestyle="--", marker="o", label=f"Projeção {horizonte}m")
    ax.fill_between(range(len(vals), len(vals) + horizonte), fut * 0.7,
                    fut * 1.3, alpha=0.2, color=COR_VERDE)
    ax.axvline(len(vals) - 1, color="gray", linestyle=":")
    ax.set_title(f"{categoria} · {nome} — {escopo}", fontweight="bold")
    ax.set_xlabel("Mês (índice)"); ax.set_ylabel("Casos"); ax.legend()
    salvar_fig(nome_arq)

    registrar_modelo(nome, categoria, "Regressão (série temporal)", metr,
                     biblioteca=f"PyTorch ({TORCH_DEVICE})",
                     observacao=f"escopo={escopo}; janela={janela}")
    return {"nome": nome, "metricas": metr, "previsoes": fut}
...
if HAS_TORCH:
    ...
    class RNN_Vanilla(nn.Module):
        """RNN simples (Elman) com ativação tanh."""
        def __init__(self, janela: int, hidden: int = 48):
            super().__init__()
    ...
    class RNN_Stacked(nn.Module):
        """RNN empilhada (3 camadas) com cabeça densa."""
        def __init__(self, janela: int, hidden: int = 48, camadas: int = 3):
            super().__init__()
            self.rnn = nn.RNN(1, hidden, num_layers=camadas, batch_first=True,
                              dropout=0.1)
            self.head = nn.Sequential(nn.Linear(hidden, 24), nn.ReLU(),
                                      nn.Linear(24, 1))
    ...
    class RNN_BiDir(nn.Module):
        """RNN bidirecional."""
        def __init__(self, janela: int, hidden: int = 48):
            super().__init__()
            self.rnn = nn.RNN(1, hidden, batch_first=True, bidirectional=True)
            self.head = nn.Linear(hidden * 2, 1)
    ...
    class RNN_LSTM_GRU(nn.Module):
        """Pilha recorrente híbrida: LSTM seguido de GRU."""
        def __init__(self, janela: int, hidden: int = 48):
            super().__init__()
            self.lstm = nn.LSTM(1, hidden, batch_first=True)
    ...
    class RNN_DeepBiGRU(nn.Module):
        """GRU bidirecional profunda (2 camadas)."""
        def __init__(self, janela: int, hidden: int = 48):
            super().__init__()
            self.gru = nn.GRU(1, hidden, num_layers=2, batch_first=True,
...
_RNN_GRANDES_MODELOS = {}
if HAS_TORCH:
    _RNN_GRANDES_MODELOS = {
        "RNN Simples (Elman)":  (RNN_Vanilla,    dict(epochs=160, lr=0.01)),
        "RNN Empilhada":        (RNN_Stacked,    dict(epochs=160, lr=0.01)),
        "RNN Bidirecional":     (RNN_BiDir,      dict(epochs=160, lr=0.01)),
        "RNN com Atenção":      (RNN_Attention,  dict(epochs=160, lr=0.01)),
        "Híbrido LSTM→GRU":     (RNN_LSTM_GRU,   dict(epochs=160, lr=0.01)),
        "BiGRU Profunda":       (RNN_DeepBiGRU,  dict(epochs=160, lr=0.01)),
    }


def executar_rnn_grandes_modelos(df: pd.DataFrame) -> dict:
    """
    MODELO 4 — RECURRENT NEURAL NETWORKS (RNNs).
    Treina a suíte de redes recorrentes sobre a série mensal de casos,
    com gráficos inline, registro de modelos e quadro comparativo.
    """
    banner_inline("MODELO 4 · RECURRENT NEURAL NETWORKS (RNNs)", cor="#512E50")
    if not HAS_TORCH:
        log.warning("  PyTorch ausente — suíte de RNN ignorada.")
        return {}
    serie, escopo = obter_serie_dl(df, janela=12)
    if len(serie) < 18:
        log.warning("  Série insuficiente para RNN.")
        return {}
    resultados = {}
    for nome, (classe, cfg) in _RNN_GRANDES_MODELOS.items():
        try:
            r = _treinar_modelo_serie(serie, escopo, nome, classe, cfg,
                                      CAT_RNN, "rnn_")
            if r:
                resultados[nome] = r
        except Exception as e:
            log.error(f"  RNN '{nome}' falhou: {e}")
    if resultados:
        rows = [[n, f"{r['metricas']['RMSE']:.2f}", f"{r['metricas']['MAE']:.2f}",
                 f"{r['metricas']['R2']:.3f}"]
                for n, r in sorted(resultados.items(),
                                   key=lambda kv: kv[1]["metricas"]["RMSE"])]
        tab = make_table(["RNN", "RMSE", "MAE", "R²"], rows,
                         col_align=["l", "r", "r", "r"])
        exibir_tabela_inline(tab, titulo="Comparativo — Recurrent Neural Networks")
        salvar_tabela_txt(tab, "rnn_comparativo",
                          "Recurrent Neural Networks — Comparativo")
        salvar_tabela_log(tab, "rnn_comparativo", "RNN Comparativo")
    log.info(f"  RNN concluído — {len(resultados)} modelos.")
    return resultados


PIPELINE_MODULES_V2.update({
    "rnn": "executar_rnn_grandes_modelos",
})


# =============================================================================
# SEÇÃO 183 ─ ARTIFICIAL NEURAL NETWORKS (ANNs) — GRANDES MODELOS
# =============================================================================
# Suíte clássica de Redes Neurais Artificiais (feedforward) para classificação
# de gravidade: Perceptron, ANN rasa, ANN profunda, ANN larga e ANN clássica
# (tanh). Contrato (B, d_in) -> (B, 1) logit.
# -----------------------------------------------------------------------------

def _classificar_torch_categoria(df: pd.DataFrame, construtores: dict,
                                 titulo: str, categoria: str,
                                 prefixo: str) -> dict:
    """Treina/avalia/registra um conjunto de redes de classificação (categoria)."""
    banner_inline(titulo, cor="#1A5276")
    if not HAS_TORCH:
        log.warning("  PyTorch ausente — suíte ignorada.")
        return {}
    df_feat = preparar_features_ml(df)
    feat_cols = [c for c in df_feat.columns
                 if c not in ["CONFIRMADO", "CASO_GRAVE", "OBITO"]]
    ...
    y = df_cls["CASO_GRAVE"].astype(int).values
    if len(X) < 200 or y.sum() < 10 or (len(y) - y.sum()) < 10:
        log.warning("  Amostra insuficiente.")
        return {}
    sc = StandardScaler()
    X = sc.fit_transform(X).astype("float32")
    X_tr, X_te, y_tr, y_te = train_test_split(
    ...
    for nome, ctor in construtores.items():
        try:
            ...
            resultados[nome] = {"metricas": metr, "hist": hist}
            registrar_modelo(nome, categoria, "Classificação (gravidade)", metr,
                             biblioteca=f"PyTorch ({TORCH_DEVICE})",
                             observacao=f"d_in={d_in}; n={len(X)}")
        except Exception as e:
            log.warning(f"  '{nome}' falhou: {e}")
    if resultados:
        rows = [[n, f"{r['metricas']['ACC']:.3f}", f"{r['metricas']['F1']:.3f}",
                 f"{r['metricas']['Recall']:.3f}",
                 (f"{r['metricas']['AUC']:.3f}"
                  if not np.isnan(r['metricas']['AUC']) else "—")]
                for n, r in sorted(resultados.items(),
                                   key=lambda kv: kv[1]["metricas"]["F1"],
                                   reverse=True)]
        tab = make_table(["Rede", "Acurácia", "F1", "Recall", "AUC"], rows,
                         col_align=["l", "r", "r", "r", "r"])
        exibir_tabela_inline(tab, titulo=f"{titulo} — Comparativo")
        salvar_tabela_txt(tab, f"{prefixo}comparativo", titulo)
        salvar_tabela_log(tab, f"{prefixo}comparativo", titulo)
        com_hist = {n: r for n, r in resultados.items() if "hist" in r}
        if com_hist:
            fig, ax = plt.subplots(figsize=(12, 5))
            for nome, r in com_hist.items():
                ax.plot(r["hist"]["val"], label=nome, linewidth=2)
            ax.set_title(f"{titulo} — Perda de Validação", fontweight="bold")
            ax.set_xlabel("Época"); ax.set_ylabel("Loss (val)"); ax.legend()
            salvar_fig(f"{prefixo}curvas")
    return resultados


if HAS_TORCH:

    class ANN_Perceptron(nn.Module):
        """Perceptron (camada linear única) — ANN mais simples."""
        def __init__(self, d_in: int):
            super().__init__()
            self.lin = nn.Linear(d_in, 1)
    ...
    class ANN_Deep(nn.Module):
        """ANN profunda (4 camadas ocultas)."""
        def __init__(self, d_in: int):
            super().__init__()
            self.net = nn.Sequential(
                nn.Linear(d_in, 128), nn.ReLU(), nn.Dropout(0.3),
                nn.Linear(128, 96), nn.ReLU(), nn.Dropout(0.2),
                nn.Linear(96, 48), nn.ReLU(),
                nn.Linear(48, 16), nn.ReLU(), nn.Linear(16, 1))

        def forward(self, x):
            return self.net(x)

    class ANN_Wide(nn.Module):
        """ANN larga (camada oculta ampla)."""
        def __init__(self, d_in: int, h: int = 512):
            super().__init__()
            self.net = nn.Sequential(nn.Linear(d_in, h), nn.ReLU(),
                                     nn.Dropout(0.4), nn.Linear(h, 1))

        def forward(self, x):
            return self.net(x)

    class ANN_Tanh(nn.Module):
        """ANN clássica com ativações tanh (multi-camada)."""
        def __init__(self, d_in: int):
            super().__init__()
            self.net = nn.Sequential(
                nn.Linear(d_in, 96), nn.Tanh(),
...
def executar_ann_grandes_modelos(df: pd.DataFrame) -> dict:
    """
    MODELO 5 — ARTIFICIAL NEURAL NETWORKS (ANNs).
    Treina a suíte clássica de ANNs feedforward para classificação de gravidade.
    """
    banner_inline("MODELO 5 · ARTIFICIAL NEURAL NETWORKS (ANNs)", cor="#1E8449")
    if not HAS_TORCH:
        return {}
    construtores = {
        "Perceptron":      lambda d: ANN_Perceptron(d),
        "ANN Rasa":        lambda d: ANN_Shallow(d),
        "ANN Profunda":    lambda d: ANN_Deep(d),
        "ANN Larga":       lambda d: ANN_Wide(d),
        "ANN Clássica (tanh)": lambda d: ANN_Tanh(d),
    }
    return _classificar_torch_categoria(
        df, construtores, "ARTIFICIAL NEURAL NETWORKS — GRAVIDADE",
        CAT_ANN, "ann_")


PIPELINE_MODULES_V2.update({
    "ann": "executar_ann_grandes_modelos",
})


# =============================================================================
# SEÇÃO 184 ─ NATURAL LANGUAGE PROCESSING (NLP) — GRANDES MODELOS
# =============================================================================
# Pipeline de PLN aplicado aos registros epidemiológicos: constrói um corpus de
# "resumos clínicos" textuais a partir das variáveis estruturadas e aplica
# frequência de termos, n-gramas e modelagem de tópicos (LDA). Bibliotecas:
# scikit-learn (CountVectorizer/TF-IDF/LDA); wordcloud (opcional).
# -----------------------------------------------------------------------------
...
STOPWORDS_PT = {
    "de", "da", "do", "com", "sem", "e", "ou", "o", "a", "os", "as", "um",
    "uma", "em", "no", "na", "paciente", "caso", "anos", "idade",
}

_SINTOMA_PALAVRA = {
    "FEBRE": "febre", "MIALGIA": "mialgia", "CEFALEIA": "cefaleia",
    "EXANTEMA": "exantema", "VOMITO": "vomito", "NAUSEA": "nausea",
    "DOR_COSTAS": "dor_nas_costas", "ARTRALGIA": "artralgia",
    "PETEQUIA_N": "petequias",
}


def construir_corpus_clinico(df: pd.DataFrame, max_docs: int = 4000) -> list:
    """
    Constrói um corpus textual de 'resumos clínicos' a partir das variáveis
    estruturadas dos casos confirmados (sintomas, classificação, perfil).
    """
    df_cg = df[(df["IS_CG"] == 1) & (df["CONFIRMADO"])].copy()
    if len(df_cg) < 30:
        df_cg = df[df["CONFIRMADO"]].copy()
    if df_cg.empty:
        return []
    if len(df_cg) > max_docs:
        df_cg = df_cg.sample(max_docs, random_state=42)

    sint_cols = [c for c in _SINTOMA_PALAVRA if c in df_cg.columns]
    corpus = []
    for _, r in df_cg.iterrows():
        termos = []
        # Perfil
        sexo = str(r.get("SEXO", "")).lower()
        if sexo in ("masculino", "feminino"):
            termos.append(sexo)
        faixa = str(r.get("FAIXA_ETARIA", "")).lower().replace(" ", "_")
        if faixa and faixa != "nan":
            termos.append("faixa_" + faixa)
        # Sintomas presentes (== 1)
        for c in sint_cols:
            try:
                if int(r[c]) == 1:
                    termos.append(_SINTOMA_PALAVRA[c])
            except Exception:
                pass
        # Classificação
        cl = str(r.get("CLASSI_DESCR", "")).lower()
        if "grave" in cl:
            termos.append("dengue_grave")
        elif "alarme" in cl:
            termos.append("dengue_alarme")
        else:
            termos.append("dengue")
        # Alarme / gravidade / hospitalização
        if int(r.get("TEM_ALARME", 0)) == 1:
            termos.append("sinais_de_alarme")
        if int(r.get("TEM_GRAVIDADE", 0)) == 1:
            termos.append("gravidade")
        if int(r.get("HOSPITALIZADO", 0)) == 1:
            termos.append("hospitalizado")
        if int(r.get("OBITO", 0)) == 1:
            termos.append("obito")
        # Sorotipo
        st = str(r.get("SOROTIPO_DESCR", "")).lower().replace("-", "")
        if st.startswith("denv"):
            termos.append(st)
        corpus.append(" ".join(termos))
    return corpus


def nlp_frequencia_termos(corpus: list) -> dict:
    """Frequência de termos do corpus (top 25) + gráfico."""
    if not _HAS_NLP or not corpus:
        return {}
    vec = CountVectorizer(stop_words=list(STOPWORDS_PT))
    X = vec.fit_transform(corpus)
    freqs = np.asarray(X.sum(axis=0)).flatten()
    termos = vec.get_feature_names_out()
    serie = pd.Series(freqs, index=termos).sort_values(ascending=False)
    top = serie.head(25)
    rows = [[t, int(v)] for t, v in top.items()]
    tab = make_table(["Termo clínico", "Frequência"], rows, col_align=["l", "r"])
    exibir_tabela_inline(tab, titulo="NLP — Frequência de Termos (top 25)")
    salvar_tabela_txt(tab, "nlp_frequencia_termos",
                      "NLP — Frequência de Termos no Corpus Clínico")
    try:
        fig, ax = plt.subplots(figsize=(11, 7))
        top.head(20).iloc[::-1].plot(kind="barh", ax=ax, color=COR_SECUNDARIA,
                                     edgecolor="black")
        ax.set_title("NLP — Termos mais frequentes (corpus clínico)",
                     fontweight="bold")
        salvar_fig("nlp_frequencia_termos")
    except Exception as e:
        log.warning(f"  Gráfico NLP freq: {e}")
    # Wordcloud opcional
    if _HAS_WORDCLOUD:
        try:
            ...
            ax.set_title("NLP — Nuvem de Palavras (corpus clínico)",
                         fontweight="bold")
            salvar_fig("nlp_wordcloud")
        except Exception as e:
            log.warning(f"  Wordcloud: {e}")
    return serie.to_dict()


def nlp_ngrams(corpus: list, n: int = 2) -> dict:
    """N-gramas mais frequentes do corpus."""
    if not _HAS_NLP or not corpus:
        return {}
    vec = CountVectorizer(ngram_range=(n, n), stop_words=list(STOPWORDS_PT))
    X = vec.fit_transform(corpus)
    freqs = np.asarray(X.sum(axis=0)).flatten()
    serie = pd.Series(freqs, index=vec.get_feature_names_out()
                      ).sort_values(ascending=False).head(15)
    rows = [[t, int(v)] for t, v in serie.items()]
    tab = make_table([f"{n}-grama", "Frequência"], rows, col_align=["l", "r"])
    exibir_tabela_inline(tab, titulo=f"NLP — {n}-gramas (top 15)")
    salvar_tabela_txt(tab, f"nlp_{n}gramas", f"NLP — {n}-gramas mais frequentes")
    return serie.to_dict()


def nlp_topicos_lda(corpus: list, n_topics: int = 5) -> dict:
    """Modelagem de tópicos por LDA sobre o corpus clínico."""
    if not _HAS_NLP or len(corpus) < 50:
        return {}
    try:
        vec = TfidfVectorizer(stop_words=list(STOPWORDS_PT), max_features=200)
        X = vec.fit_transform(corpus)
        ...
        for i, comp in enumerate(lda.components_):
            ...
            topicos[f"Tópico {i+1}"] = palavras
            rows.append([f"Tópico {i+1}", ", ".join(palavras)])
        tab = make_table(["Tópico", "Termos principais"], rows,
                         col_align=["l", "l"], max_width=120)
        exibir_tabela_inline(tab, titulo=f"NLP — Tópicos (LDA, k={n_topics})")
        salvar_tabela_txt(tab, "nlp_topicos_lda",
                          "NLP — Modelagem de Tópicos (LDA)")
        salvar_tabela_log(tab, "nlp_topicos_lda", "NLP Tópicos LDA")
        registrar_modelo(f"LDA Topic Model (k={n_topics})", CAT_NLP,
                         "Modelagem de tópicos",
                         {"perplexidade": float(lda.perplexity(X))},
                         biblioteca="scikit-learn",
                         observacao=f"docs={len(corpus)}; termos={len(termos)}")
        return topicos
    except Exception as e:
        log.warning(f"  LDA falhou: {e}")
        return {}


def executar_nlp(df: pd.DataFrame) -> dict:
    """
    MODELO 6 — NATURAL LANGUAGE PROCESSING (NLP).
    Constrói o corpus clínico e executa frequência de termos, n-gramas e LDA.
    """
    banner_inline("MODELO 6 · NATURAL LANGUAGE PROCESSING (NLP)", cor="#6C3483")
    if not _HAS_NLP:
        log.warning("  scikit-learn (text) indisponível — NLP ignorado.")
        return {}
    corpus = construir_corpus_clinico(df)
    if not corpus:
        log.warning("  Corpus clínico vazio — NLP ignorado.")
        return {}
    log.info(f"  Corpus clínico: {len(corpus)} documentos (resumos de casos).")
    res = {}
    res["frequencia"] = nlp_frequencia_termos(corpus)
    res["bigramas"] = nlp_ngrams(corpus, n=2)
    res["trigramas"] = nlp_ngrams(corpus, n=3)
    res["topicos"] = nlp_topicos_lda(corpus, n_topics=5)
    # Exporta o corpus
    try:
        pd.DataFrame({"resumo_clinico": corpus}).to_csv(
            OUTPUT_DIR / "dados" / "nlp_corpus_clinico.csv", index=False,
            encoding="utf-8-sig")
        log.info("  [CSV] nlp_corpus_clinico.csv")
    except Exception as e:
        log.warning(f"  CSV corpus: {e}")
    log.info("  NLP concluído.")
    return res


PIPELINE_MODULES_V2.update({
    "nlp":            "executar_nlp",
    "corpus_clinico": "construir_corpus_clinico",
    "nlp_topicos":    "nlp_topicos_lda",
})


# =============================================================================
# SEÇÃO 185 ─ ORQUESTRADOR PRINCIPAL v1.3 (main_v4)
# =============================================================================

def executar_extras_v4(df: pd.DataFrame) -> dict:
    """
    Camada ADICIONAL da v1.3:
      • MODELO 4 — Recurrent Neural Networks (RNNs)
      • MODELO 5 — Artificial Neural Networks (ANNs)
      • MODELO 6 — Natural Language Processing (NLP)
    """
    banner_inline("SIPREV v1.3 — CAMADA RNN · ANN · NLP", cor="#6E2C00")
    res = {}
    res["rnn"] = executar_rnn_grandes_modelos(df)
    res["ann"] = executar_ann_grandes_modelos(df)
    res["nlp"] = executar_nlp(df)
    n_rnn = len(obter_modelos_por_categoria(CAT_RNN))
    n_ann = len(obter_modelos_por_categoria(CAT_ANN))
    n_nlp = len(obter_modelos_por_categoria(CAT_NLP))
    log.info(f"  v1.3 — RNN: {n_rnn} · ANN: {n_ann} · NLP: {n_nlp} modelos.")
    return res


def main_v4():
    """
    PIPELINE COMPLETO v1.3 — SIPREV.

    Sequência:
      1) garantir_dados_local()       — localiza ou BAIXA os CSVs (barra de progresso)
      2) compilar_300_bibliotecas()   — 100 ML + 100 DL + 100 NN
         + documentacao_completa_v2()
      3) main_max()                   — pipeline base completo
      4) executar_modelos_v2(df)      — inteligência v1.0 (ML/DL/NN, redes, etc.)
      5) executar_extras_v3(df)       — camada v1.2 (DL/ML/NN extras, preditiva,
                                        prevenção, processamento, documentação)
      6) executar_extras_v4(df)       — RNN · ANN · NLP (v1.3)
      7) relatórios consolidados finais + fechamento
      8) exportacao_completa_v2() + exportar_zip_v2()
         → SIPREV_DENG_MS_EpiAnalysis_<data_hora>.zip
    ...
    t0 = time.time()
    limpar_registro_modelos()
    banner_inline("SIPREV v1.3 — PIPELINE COMPLETO (300 BIBLIOTECAS · "
                  "ML/DL/NN/RNN/ANN/NLP · PREVISÃO/PREDIÇÃO/PREVENÇÃO)",
                  cor="#7B241C")
    ...
    # 2) Catálogos (300) + documentação técnica
    try:
        compilar_300_bibliotecas()
    except Exception as e:
        log.error(f"  compilar_300_bibliotecas falhou: {e}")
    try:
        documentacao_completa_v2()
    except Exception as e:
        log.error(f"  documentacao_completa_v2 falhou: {e}")
    ...
    # 4–7) Inteligência (v1.0 + v1.2 + v1.3) + consolidação
    resultados = {}
    if df is not None and not df.empty:
        for etapa, fn in [("v2", "executar_modelos_v2"),
                          ("v3", "executar_extras_v3"),
                          ("v4", "executar_extras_v4")]:
            try:
                resultados[etapa] = globals()[fn](df)
            except Exception as e:
                log.error(f"  {fn} falhou: {e}")
                traceback.print_exc()
        for fn in ("relatorio_todos_modelos_treinados", "leaderboard_unificado",
                   "comparar_categorias_modelos", "exportar_dashboard_modelos_html",
                   "documentacao_extra_final", "tabela_codigos_sinan",
    ...
    dur = time.time() - t0
    banner_inline("SIPREV v1.3 — EXECUÇÃO CONCLUÍDA", cor="#196F3D")
    log.info(f"  Duração total      : {dur/60:.1f} min ({dur:.0f}s)")
    log.info(f"  Modelos treinados  : {len(MODELOS_TREINADOS)}")
    log.info(f"  Gráficos gerados   : {_exec_stats.get('graficos_gerados', 0)}")
    log.info(f"  Relatórios gerados : {_exec_stats.get('relatorios_gerados', 0)}")
...
PIPELINE_MODULES_V2.update({
    "extras_v4":   "executar_extras_v4",
    "pipeline_v4": "main_v4",
})


# =============================================================================
# SEÇÃO 186 ─ ESPECIFICAÇÃO TÉCNICA v1.3 (DOCUMENTAÇÃO FINAL)
# =============================================================================
"""
ESPECIFICAÇÃO TÉCNICA — SIPREV v1.3
==================================

NOVIDADES DA v1.3 (em relação à v1.2)
-------------------------------------
 • Catálogos ampliados para 300 bibliotecas: 100 Machine Learning + 100 Deep
   Learning + 100 Neural Networks (com detecção de versão e exportação).
 • MODELO 4 — Recurrent Neural Networks (RNNs): RNN simples (Elman), empilhada,
   bidirecional, com atenção, híbrido LSTM→GRU e BiGRU profunda.
 • MODELO 5 — Artificial Neural Networks (ANNs): Perceptron, ANN rasa, profunda,
   larga e clássica (tanh) para classificação de gravidade.
 • MODELO 6 — Natural Language Processing (NLP): construção de corpus de resumos
   clínicos a partir das variáveis estruturadas; frequência de termos; n-gramas;
   modelagem de tópicos (LDA); nuvem de palavras (opcional).
 • Orquestrador main_v4 integrando base + v1.0 + v1.2 + v1.3.

PESQUISA SOBRE TECNOLOGIA EMERGENTE
-----------------------------------
 Este programa serve de base para um artigo sobre tecnologias emergentes em
 saúde pública: integra IA (ML/DL/RNN/ANN), PLN aplicado a dados de vigilância,
 análise de redes (NetworkX) e modelagem preditiva com avaliação temporal. As
 funções gera_rascunho_artigo() e relatorio_tecnico_completo() produzem material
 inicial para a redação.

ENTRADA PRINCIPAL
-----------------
 • main_v4()              → pipeline completo v1.3
...
COMPATIBILIDADE
---------------
 • Python 3.12 / 3.13 / 3.14; Anaconda. Local · Google Colab · Google Cloud.
 • DL/RNN/ANN/NLP usam PyTorch + scikit-learn (independem do TensorFlow).
 • Bibliotecas opcionais detectadas em runtime (degradação segura).
"""

log.info("=" * 78)
log.info("  SIPREV v1.3 — expansão completa (RNN · ANN · NLP · 300 bibliotecas).")
log.info(f"  Módulos registrados: {len(PIPELINE_MODULES_V2)}")
log.info("  Entrada principal: main_v4()  |  Demo: executar_demo_sintetica()")
log.info("=" * 78)


# =============================================================================
# PONTO DE ENTRADA v1.3 — executa o pipeline completo (main_v4)
# =============================================================================
